...
module "kms_cloudwatch_log_group" {
  source         = "github.com/cloudbackenddev/kube-common//terraform-modules/aws/kms/cloudwatch_log_group"
...
module "eks" {
  ...
  control_plane_subnet_ids = length(var.private_subnets) > 0 ? var.private_subnets : var.k8s_subnets
